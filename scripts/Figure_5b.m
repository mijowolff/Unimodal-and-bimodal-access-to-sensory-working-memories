...
close all;
clc;
%% Figures 5b, from Unimodal and bimodal access to WM (2019) Wolff et al.
...
if do_decoding
    for sub=1:28
        ...
        incl=intersect(setdiff(1:size(Results,1), find(bad_ind_mem==1)),incl_v);
        ...
        for rep=1:params.reps
            distance_b = mahal_func_theta_kfold_b_sep(data_v,data_m,theta,params.n_folds);
            temp_c(rep,1)=mean(distance_b,2);
        end
        ...
        temp_c=nan(params.reps,1);
        for rep=1:params.reps
            distance_b = mahal_func_theta_kfold_b_sep(data_m,data_v,theta,params.n_folds);
            temp_c(rep,1)=mean(distance_b,2);
        end
        cued_imp_vis_m_dec(sub,1)=mean(temp_c,1);
    end
    cued_imp_vis_dec_b=(cued_imp_vis_dec+cued_imp_vis_m_dec)./2; average
else
    load('Figure_5b_results')
end
