...
close all;
clc;
%% Figures 5a left, from Unimodal and bimodal access to WM (2019) Wolff et al.
...
if do_decoding
    for sub=1:30
        ...
        % cued item; train on item presentation, test on auditory impulse
        ...
        for rep=1:params.reps
            distance_b = mahal_func_ordinal_kfold_b_sep(data_a,data_m,conds,params.n_folds);
            temp_b(rep,1)=mean(distance_b,1);
        end
        cued_imp_aud_dec(sub,1)=mean(temp_b,1);
        
        % cued item; train on auditory impulse, test on item presentation
        conds=round(log2((Results(incl,4))./270).*2)+1; % cued item frequency, convert to condition labels
        temp_b=nan(params.reps,1);
        for rep=1:params.reps
            distance_b = mahal_func_ordinal_kfold_b_sep(data_m,data_a,conds,params.n_folds);
            temp_b(rep,1)=mean(distance_b,1);
        end
        cued_imp_aud_m_dec(sub,1)=mean(temp_b,1);
    end
    cued_imp_aud_dec_b=(cued_imp_aud_dec+cued_imp_aud_m_dec)./2; % average
else
    load('Figure_5a_left_results')
end
%% significance testing
n_perms=100000;
p_aud_cued=GroupPermTest(cued_imp_aud_dec_b,100000,2);
%%
ci_impa_cued=bootci(100000,@mean,cued_imp_aud_dec_b);
%% plot boxplots and error bars
pos=[1];
figure
title('Auditory WM')
hold all
b1=boxplot([cued_imp_aud_dec_b],...
    'positions',pos,'Widths',0.1,'Symbol','ko','Labels',{'aud. imp. & item pres.'});
set(findobj(gcf,'LineStyle','--'),'LineStyle','-')
set(b1(:,1),'color','b');
plot(pos(1),mean(cued_imp_aud_dec_b,1),'o','MarkerFaceColor','b','MarkerEdgeColor','none','MarkerSize',10)
