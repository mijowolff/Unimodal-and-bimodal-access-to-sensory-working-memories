...
close all;
clc;
%% Figures 3f, from Unimodal and bimodal acces to WM (2019) Wolff et al.
...
if do_decoding
    for sub=1:28
        ...
        % cued item
        theta=Results(incl,4).*2; % cued item orientation
        temp_c=nan(params.reps,1);
        for rep=1:params.reps
            distance_c = mahal_func_theta_kfold_b(data,theta,params.n_folds);
            temp_c(rep,1)=mean(distance_c,2);
        end
        cued_imp_aud_dec(sub,1)=mean(temp_c,1);
        
        % uncued item
        theta=Results(incl,5).*2; % uncued item orientation
        temp_c=nan(params.reps,1);
        for rep=1:params.reps
            distance_c = mahal_func_theta_kfold_b(data,theta,params.n_folds);
            temp_c(rep,1)=mean(distance_c,2);
        end
        uncued_imp_aud_dec(sub,1)=mean(temp_c,1);
        ...
        cued_imp_vis_dec(sub,1)=mean(temp_c,1);
        ...
        uncued_imp_vis_dec(sub,1)=mean(temp_c,1);
    end
else
    load('Figure_3f_results')
end
...
VIS_WM_imps_table=table(cued_imp_aud_dec,uncued_imp_aud_dec,...
    cued_imp_vis_dec,uncued_imp_vis_dec,...
    'VariableNames',{'imp_aud_cued','imp_aud_uncued','imp_vis_cued','imp_vis_uncued'});
writetable(VIS_WM_imps_table,fullfile([pwd '\results\'],'VIS_WM_imps_table.txt'),'Delimiter',' ')
%% make C.I. for plots
